...
@endsection
@section('content')
{{ Form::open(array('url'=>'menu/add','method'=>'post','class'=>'definewidth m20')) }} 
<table class="table table-bordered table-hover ">
 <tr>
        <td width="10%" class="tableleft">排序值</td>
        <td>{{ Form::text('menu_order') }}(值越大，排序越靠前) </td>
        
    </tr>
    <tr>
        <td class="tableleft">菜单名</td>
        <td >{{ Form::text('menu_name') }}</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td >{{ Form::text('menu_url') }}</td>
    </tr>    
    <tr>
        <td class="tableleft">上级菜单</td>
        <td >
        {{Form::select('menu_parent', $menu_parentList) }}
        </td>
    </tr>
    <tr>
        <td class="tableleft">是否启用</td>
        <td >
        {{Form::select('menu_flag', $menu_flag) }}
        </td>
    </tr>
    <tr>
