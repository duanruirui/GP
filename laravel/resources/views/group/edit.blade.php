...
@endsection
@section('content')
<form method='post' action="{{ url('group/edit/'.$row->f_group_id) }}" class='efinewidth m20'>
{{ csrf_field() }}
<table class="table table-bordered table-hover ">
    <tr>
        <td width="10%" class="tableleft">角色名称</td>
        <td><input type='text' name='group_name' value='{{ $row->f_group_name }}'>&nbsp;&nbsp;<span class='msg'>@if ($errors->has('group_name')) {{ $errors->first('group_name') }} @endif</span></td>
        
    </tr>
    <tr>
        <td class="tableleft">角色说明</td>
        <td ><textarea type='text' name='group_describe' value='{{ $row->f_group_name }}'>{{ $row->f_group_describe }}</textarea></td>
    </tr>  
